...
RSpec.describe Auth::JsonWebTokenService do
  ...
  context 'secret key was assigned' do
    describe '.encode' do
  ...
  context 'secret key was not assigned' do
    before { stub_const('Auth::JsonWebTokenService::HMAC_SECRET', nil) }

    describe '.encode' do
      it 'raises InvalidSecretKey error' do
        expect { described_class.encode(payload) }.to raise_error(ExceptionHandler::InvalidSecretKey, /secret_key_base not assigned/)
      end
    end

    describe '.decode' do
      it 'raises InvalidSecretKey error' do
        expect { described_class.decode(fake_token) }.to raise_error(ExceptionHandler::InvalidSecretKey, /secret_key_base not assigned/)
      end
    end
  end
end
